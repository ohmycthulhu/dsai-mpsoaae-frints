...
long TEN_POWERS_UNSIGNED[] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000};
...
inline long powerTenDigits(u_short deg) {
    return TEN_POWERS_UNSIGNED[deg];
}
...
float Frint::toFloat() const {
    return ((float)this->mSignificand) * powerTen(this->mExponent) / powerTenDigits(this->mNbDigits);
}

double Frint::toDouble() const {
    return ((double)this->mSignificand) * powerTen(this->mExponent) / powerTenDigits(this->mNbDigits);
}
...
Frint Frint::operator+(const Frint& other) const {
    ...
    long first = this->mSignificand, second = other.mSignificand;

    if (this->mNbDigits > other.mNbDigits) {
        second = shiftLeft(second, this->mNbDigits - other.mNbDigits);
    } else if (this->mNbDigits < other.mNbDigits) {
        first = shiftLeft(first, other.mNbDigits - this->mNbDigits);
    }

    if (this->mExponent > other.mExponent) {
        second = shiftRight(second, this->mExponent - other.mExponent);
    }

    long significand = first + second, exponent = this->mExponent + (numberOfDigits(significand) - numberOfDigits(first));
...
Frint Frint::operator*(const Frint& other) const {
    long int fSignificand, sSignificand;

    this->normalizeNumbers(*this, other, &fSignificand, &sSignificand);
    long int significand = fSignificand * sSignificand;
    long int exponent = this->mExponent + other.mExponent - (
        (numberOfDigits(fSignificand) + numberOfDigits(sSignificand)) - numberOfDigits(significand)
    );
...
Frint Frint::operator~() const {
    long divider = this->mNbDigits <= 3 ? this->mSignificand : this->shiftRight(this->mSignificand, this->mNbDigits - 3);
...
long int Frint::shiftLeft(long int number, u_short nb) const {
    return nb != 0 ? number * powerTenDigits(nb) : number;
}
